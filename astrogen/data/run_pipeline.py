...
def load_people(*args):
    script = 'CREATE TABLE IF NOT EXISTS people ('+', '.join(D.columns)+')'
    c.execute(script)
    conn.commit()
    D.to_sql('people', conn, if_exists='replace', index = False)    
...
if __name__ == '__main__' and '__file__' in globals():
    ...
    D = S04_make_pages(df15); Dpages = next(D)
 
    D = S04_pub_filter_criteria_apply(df15); df16 = next(D)
...
if __name__ == '__main0__' and '__file__' in globals():

    conn = sqlite3.connect('../../data/redux/astrogen_DB_rc3.db')
    c = conn.cursor()
